...
//    }
//
//    wchar_t *unicodeStr=(wchar_t *)kmalloc(sizeof(wchar_t)*(unicodeLen+1), GFP_ATOMIC);
...
unsigned char GB2312Tables[100][6][16][5] = {
    ...
    {{" ", "！", "＂", "＃", "￥", "％", "＆", "＇", "（", "）", "＊", "＋", "，", "－", "．", "／"},//A3A0
     {"０", "１", "２", "３", "４", "５", "６", "７", "８", "９", "：", "；", "＜", "＝", "＞", "？"},  //A3B0
     {"＠", "Ａ", "Ｂ", "Ｃ", "Ｄ", "Ｅ", "Ｆ", "Ｇ", "Ｈ", "Ｉ", "Ｊ", "Ｋ", "Ｌ", "Ｍ", "Ｎ", "Ｏ"},  //A3C0
     {"Ｐ", "Ｑ", "Ｒ", "Ｓ", "Ｔ", "Ｕ", "Ｖ", "Ｗ", "Ｘ", "Ｙ", "Ｚ", "［", "＼", "］", "＾", "＿"},  //A3D0
     {"｀", "ａ", "ｂ", "ｃ", "ｄ", "ｅ", "ｆ", "ｇ", "ｈ", "ｉ", "ｊ", "ｋ", "ｌ", "ｍ", "ｎ", "ｏ"},  //A3E0
     {"ｐ", "ｑ", "ｒ", "ｓ", "ｔ", "ｕ", "ｖ", "ｗ", "ｘ", "ｙ", "ｚ", "｛", "｜", "｝", "￣", " "}},  //A3F0
...
void Url_Decode_GB2312(const char *src, char *pout)
{
    ...
    int i = 0,j,k,n,s, w = 0;
    char a[4]={0};
    char *bchar = (char *)kmalloc(sizeof(char)*1, GFP_ATOMIC);
    while(*p != '\0')
    {
        a[0]=*(p);
        a[1]=*(p+1);
        a[2]='\0';
        if (!(strcmp(a,"AA") == 0 || strcmp(a,"AB") == 0 || strcmp(a,"AC") == 0 || strcmp(a,"AD") == 0 || strcmp(a,"AE") == 0 || strcmp(a,"AF") == 0))
        {
            for (j = 0; strcmp(GB2312line1[j],"00") != 0  ; ++j)
            {
                if(strcmp(a,GB2312line1[j]) == 0)
                {
//                    printf("%s", GB2312line1[j]);
                    for (k = 0; GB2312line2[k] != '\0'; ++k)
                    {
                        if(*(p+2) == GB2312line2[k])
                        {
//                            printf("%c", GB2312line2[k]);
                            for (n = 0; GB2312line3[n] != '\0'; ++n)
                            {
                                if(*(p+3) == GB2312line3[n])
                                {
//                                    printf("%c:\t%s\n", GB2312line3[n], GB2312Tables[j][k][n]);
            ...
            if (strcmp(GB2312line1[j],"00") == 0) {
//                pout[i++] = *p;
//                pout[i++] = *(p+1);
//                printk("%c%c\t", *p, *(p+1));
                HexStrToByte(p, bchar, 2);
                pout[i++] = *bchar;
                p += 2;
                continue;
            }
...
//%B6%AB%DD%B8%B8%CA%D0%C7%E5%CF%AA%AA%D5%F2%BD%CC%D3%FD%D0%C5%CF%A2%A2%CD%F8
//B6ABDDB8B8CAD0C7E5CFAAAAD5F2BDCCD3FDD0C5CFA2A2CDF8
//int main(int argc, char const *argv[])
//{
//    char *s="B6ABDDB8B8CAD0C7E5CFAAAAD5F2BDCCD3FDD0C5CFA2A2CDF8";
//    char out[1024]={0};
//
