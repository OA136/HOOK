...
unsigned int hook_func(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
{
	...
	if (iph->protocol == IPPROTO_TCP)
	{
		...
		if (sport == 80)
		{
			...
			{
				char *p,*p1,*page, *k, *chset_start, *chset_end;
...
//                        kfree(dest);
                        ...
                        char *src =  (char *)kmalloc(sizeof(char)*(codeLen + 8)*2, GFP_ATOMIC);
                        //解码gb2312
                        if (strcmp(charset, "gb2312") == 0 || strcmp(charset, "GB2312") == 0 || strcmp(charset, "gbk") == 0 || strcmp(charset, "GBK") == 0) {
                            int i = 0;
                            for (i = 0;i < codeLen + 7;i++) {
                                sprintf(src+2*i, "%02X", ppage[i]&0xFF);
                            }
                            *(src+2*i) = '\0';
...
//            char *request = strstr(pkg,"HTTP/1.1");
//            if(request == NULL) return NF_ACCEPT;
...
//                printk("ajax虎门教育网\n:%s", request);
				delete_accept_encoding(pkg);
				printk(KERN_ALERT "Found ajax 请求\n");
