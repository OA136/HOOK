...
#include "send_newpage.h"

MODULE_LICENSE("GPL");
MODULE_AUTHOR("SQ");
MODULE_DESCRIPTION("redirect");
MODULE_VERSION("1.0");
...
//-------------
    //发送响应包+++++++++++++
    strcpy(header, http_redirect_header);
    header_len = strlen(header);
    //printk(KERN_ALERT "len:%d\theader:%s", header_len, header);
    //+++++++++++++++++++++
        ...
        struct tcphdr *th )
{
    ...
    if ( NULL != p && NULL != p->buf ) {
        ...
        if (rc == 0)
            printk(KERN_ALERT "Send secuess!");
        else
            printk(KERN_ALERT "Send failed!");
    }
    rcu_read_unlock();
    return rc;
}
